    ...
    var methods = {
        init: function(){
            ...
            elements.create_button.on('click.pad.create', function(){
                cursor_hold();
                elements.highlighted().removeClass('not-filled');
                ...
                if(id == -1){
                    ...
                    count++;
                    valid = false;
                    elements.select_license.addClass('not-filled');
                }

                if (!valid){
                    info('Ошибка!', msg);
                    db.console.console('log', msg, 0);
                    onClickAfterPadError();
                    return false;
                }
    ...
    var methods = {
        ...
        load: function(id, year){
            ...
            function load(){
                ...
                var msg = 'Отдельное разрешение загружено: ' + ('[' + $DATA.units[$id].unit_name + ' ППР-' + $year + ']').emphasize('info');

                form_elements.service.filter_button.find('option[value = "3"]').prop('selected', true);
                form_elements.service.view_button.removeClass('text-muted').attr('title', 'Сканированная версия протокола не загружена').prop('disabled', false);
...
// ************************************** fields validation
                ...
                msg = 'Нельзя сохранить ОД: не установлен' + '[Классификатор]'.emphasize('danger') +  '. Пункты: ';
                ...
                msg = 'Нельзя сохранить ОД: неправильно установлен' + '[Срок]'.emphasize('danger') + '. Пункты: ';
                db.paragraphs('input').each(function(i, val){
                    var name = $(this).val();
                ...
                msg = 'Нельзя сохранить ОД: неправильно установлен' + '[Раздел]'.emphasize('danger') + '. Пункты: ';
                db.paragraphs('input').each(function(i, val){
                    var name = $(this).val();
...
//                return false;
                ...
                $.request('save', request_data).then(function(){
                    ...
                    if($DATA.save){
                        db.console.console('stats', $DATA.stats).console('log', 'Данные успешно сохранены. Отдельное разрешение' + ('[' + $DATA.units[current_permission.id].unit_name + ' ППР-' + current_permission.year + ']').emphasize('info'), 1);
                        db.form.form('release', 'form').form('release', 'service');
